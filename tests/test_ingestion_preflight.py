...
def test_preflight_is_read_only_and_reports_local_readiness(tmp_path):
    ...
    assert result["report_discovery_state"]["queue"]["due_now"] == 1
    assert {path: path.read_bytes() for path in before} == before

    writer_lock.write_text(
        '{"run_id":"active","mode":"incremental"}', encoding="utf-8"
    )
    locked = build_ingestion_preflight(
        committed_manifest_path=manifest,
        refresh_state_path=refresh,
        report_discovery_state_path=discovery,
        report_detail_state_path=details,
    ...
    assert locked["checks"]["no_active_writer"] is False
    assert locked["checks"]["ready_for_explicit_ingestion"] is False
    assert locked["writer_lock"]["run_id"] == "active"
